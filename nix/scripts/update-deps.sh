...
# 2. Pins cabal.project's index-state to the update time
# 3. Syncs stack.yaml snapshot with nixpkgs LTS version
# 4. Checks Hackage for new Cabal versions
# 5. Regenerates nix package definitions
...
# Taken before the update so the pin is never newer than the fetched index
index_state=$(date -u +%Y-%m-%dT%H:%M:%SZ)

nix flake update &
flake_update_pid=$!
cabal update &
cabal_update_pid=$!
wait "$flake_update_pid" || { echo "Failed to update flake"; exit 1; }
wait "$cabal_update_pid" || { echo "Failed to update cabal"; exit 1; }
...
EOF
echo "Pinned cabal.project to index-state $index_state"
...
# Extract amazonka commit from stack.yaml. (Amazonka is pinned to git
# main: GHC 9.10 support is not yet released.)
...
# Note [Riding latest Cabal]
# ~~~~~~~~~~~~~~~~~~~~~~~~~~
# The whole point of this tool is to parse every cabal file on Hackage,
# so Cabal and Cabal-syntax pin the latest Hackage release, ahead of
# GHC, nixpkgs, and the Stackage snapshot. That outruns parts of the
# ecosystem, patched in flake.nix's overlay and stack.yaml's extra-deps:
#  - Cabal's process bounds are ignored; they only exclude releases
#    that segfault on macOS (haskell/cabal#11465), and these builds are
#    Linux-only.
#  - hackage-security and cabal-doctest also pin latest: snapshot and
#    nixpkgs versions predate Cabal(-syntax) 3.18.
#  - entropy's and ghc-paths's custom Setup.hs's predate Cabal 3.18's
#    Verbosity changes. The nix build compiles them with GHC's bundled
#    Cabal; stack pins a patched entropy (haskell/entropy#97).
...
gen_pkg hackage-security cabal://hackage-security &
pid11=$!
